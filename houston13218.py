...
# mcc loss
parser.add_argument('--temperature', default=8.0,
...
for iDataSet in range(nDataSet):
    ...
    data_st = utils.load_data_houston_st(data_path_s,data_path_t)
    ...
    data_s = np.concatenate((data_st, data_s), axis=2)
    ...
    # EMA teacher
    F1_teacher = EMATeacher(G, F1, alpha=args.alpha, pseudo_label_weight=args.pseudo_label_weight).to(
        device)  # alpha=0.9
    lr = args.lr
    if args.cuda:
        G.cuda()
    ...
    if args.optimizer == 'momentum':
        ...
        optimizer_f = optim.SGD(list(F1.parameters()), momentum=0.9, lr=args.lr,
                                weight_decay=0.0005)
    elif args.optimizer == 'adam':
        optimizer_g = optim.Adam(G.features.parameters(), lr=args.lr, weight_decay=0.0005)
        optimizer_f = optim.Adam(list(F1.parameters()), lr=args.lr, weight_decay=0.0005)
    else:
        optimizer_g = optim.Adadelta(G.features.parameters(), lr=args.lr, weight_decay=0.0005)
        optimizer_f = optim.Adadelta(list(F1.parameters()), lr=args.lr, weight_decay=0.0005)

    memory = torch.zeros((num_classes, 152), dtype=torch.float32).cuda()
    ema_alpha = 0.9
    def train(ep, train_loader, train_tar_loader):
        ...
        for batch_idx in range(1, num_iter):
            ...
            data_source_all, label_source = next(iter_source)
            data_source = data_source_all[:, :48, :, :]
            data_source_st = data_source_all[:, 48:, :, :]
            data_target, _ = next(iter_target)

            if args.cuda:
                data1, target1 = data_source.cuda(), label_source.cuda()
                data2 = data_target.cuda()
                data3, target3 = data_source_st.cuda(), label_source.cuda()
            ...
            F1_pseudo_label_1, F1_pseudo_prob_1 = F1_teacher(data1)
            F1_pseudo_label_2, F1_pseudo_prob_2 = F1_teacher(data2)
            F1_pseudo_label_3, F1_pseudo_prob_3 = F1_teacher(data3)
            ...
            output1 = G(data1)
            output1_F = F1(output1)
            output_1 = output1_F[0][:batch_size, :]
            # output2 = G(data2)
            # output2_F = F1(output2)
            # output_2 = output2_F[0][:batch_size, :]
            class_prototypes = np.zeros((num_classes, 152))
            feature_s = output1[:32, :]

            for i in range(num_classes):
                class_samples = feature_s[target1 == i]
                if class_samples.size(0) > 0:
                    class_samples = class_samples.cpu()
                    class_prototypes[i] = class_samples.detach().mean(dim=0)
            class_prototypes = torch.tensor(class_prototypes, dtype=torch.float32)

            if batch_idx == 1:
                memory = class_prototypes.detach().clone()
            else:
                memory = ema_alpha * memory + (1 - ema_alpha) * class_prototypes


            output3 = G(data3)
            output3_F = F1(output3)
            output_3 = output3_F[0][:batch_size, :]

            loss1 = F.cross_entropy(output_1, target1.long().cuda())
            loss2 = F.cross_entropy(output_3, F1_pseudo_label_1.long())
            loss3 = F.cross_entropy(output_1, F1_pseudo_label_3.long())

            all_loss = loss1 + (loss2 + loss3*0.6)*0.5

            if args.log_results:
                wandb.log({f'{seeds[iDataSet]}_A_loss': all_loss,
                           f'{seeds[iDataSet]}_A_mask loss': 0})
            all_loss.backward()
            optimizer_g.step()
            optimizer_f.step()
            ...
            feature_t = output2[:32, :]
            memory = torch.tensor(memory, dtype=torch.float32)
            distances = torch.cdist(feature_t.cuda(), memory.cuda())
            negative_distances = -distances
            confidence = F.softmax(negative_distances, dim=1)
            confidence1, no = torch.max(confidence, dim=1)
            pseudo_labels = torch.argmin(distances, dim=1)
            result = compare_pseudo_labels(F1_pseudo_label_2.cuda(), pseudo_labels.cuda())

            loss4 = 0
            ce = F.cross_entropy(output_2, F1_pseudo_label_2.long().cuda(), reduction='none').float()
            loss4 += torch.mean(ce * result.float() * confidence1)
            F_loss = loss4*0.6

            if args.log_results:
                wandb.log({f'{seeds[iDataSet]}_A_loss': F_loss,
                           f'{seeds[iDataSet]}_A_mask loss': 0})
            F_loss.backward()
            optimizer_f.step()
            ...
            if batch_idx % args.log_interval == 0:
                print(
                    'Train Ep: {} [{}/{} ({:.0f}%)]\tLoss1: {:.6f}\tall_loss: {:.6f}\t Dis: {:.6f} MCC: {:.6f}'.format(
                        ep, batch_idx * len(data), args.batch_size * len(train_loader),
                            100. * batch_idx / len(train_loader), loss1.item(), all_loss.item(), 0,
                        0))
    ...
    def test(test_loader):
        ...
        with torch.no_grad():
            ...
            print('\nTest set: Average loss: {:.4f}, Accuracy: {}/{} ({:.2f}%) \n'.format(
                ...
                100. * correct / len_tar_dataset))
            value = 100. * correct / len_tar_dataset
            if args.log_results:
                wandb.log({f'{seeds[iDataSet]}_result_1': 100. * correct / len_tar_dataset})
        return value, pred1_list, label_list, acc1, outputdata, label_list, predict, labels
    ...
    for ep in range(1, num_epoch + 1):
        ...
        if ep % num_epoch == 0:
            ...
            value, pred1_list, label_list, acc1, outputdata_target, target_label, predict, labels = test(
                test_loader)
            test_end = time.time()
            acc[iDataSet] = acc1
